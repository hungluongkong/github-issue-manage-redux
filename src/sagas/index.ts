...
export function* fetchIssuesHandler() {
  try {
    ...
    if (response.status === ResponseCode.OK && response.data) {
      yield put(getIssueSuccess(RawToIssue(response.data)));
    } else {
...
export function* fetchIssuesSaga() {
  yield takeEvery(Action.GET_ISSUES, fetchIssuesHandler);
}

export function* handleIssueUpdate(action: ActionPayload) {
  try {
    const method: RequestMethod = action.issue?.id ? api.patch : api.post;
    const response: ApiResponse<RawIssue & { message: string }> = yield method(
      action.issue?.url || '',
      action.issue,
    );
    ...
    if (
      ...
      yield put(
        getIssueError(
          response.data?.message || localize.en.ERROR_SOMETHING_WRONG,
        ),
      );
    }
  } catch (error) {
...
export function* updateIssueSaga() {
  yield takeEvery<string, any>(Action.UPDATE_ISSUE, handleIssueUpdate);
}
...
export function* handleToggleIssue(action: ActionPayload) {
  try {
    ...
    const response = yield method(`${issue.url}/lock`, {
      locked: !issue.locked,
    });

    // Update issue if it ok.
    if (response.status === ResponseCode.NO_CONTENT) {
      yield put(updateIssueSuccess({ ...issue, locked: !issue.locked }));
    } else {
      yield put(getIssueError(response.data.message));
    }
  } catch (error) {
...
export function* toggleIssueSaga() {
  yield takeEvery<string, any>(Action.TOGGLE_LOCK_ISSUE, handleToggleIssue);
}

// Root sagas
export default function* rootSaga() {
  yield all([fetchIssuesSaga(), updateIssueSaga(), toggleIssueSaga()]);
}
